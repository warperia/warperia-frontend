...
// Addon Modals
const SwitchVariationModal = lazy(() => import('../components/Modals/SwitchVariationModal.js'));
const AddonSelectionModal = lazy(() => import('../components/Modals/AddonSelectionModal.js'));
...
const AddonsPage = ({ user, currentExpansion, serverPath, gamePath, activeTab, setActiveTab }) => {
    ...
    const [contextMenu, setContextMenu] = useState({ visible: false, xPos: 0, yPos: 0, addon: null });
    const [showModal, setShowModal] = useState(false);
    const [addonLoading, setAddonLoading] = useState(false);
    const [selectedAddon, setSelectedAddon] = useState(null);
    const [categories, setCategories] = useState([]);
    const [selectedCategories, setSelectedCategories] = useState([]);
    const [selectedSorting, setSelectedSorting] = useState({ value: 'installs', label: 'Most Popular' });
    const [previousPage, setPreviousPage] = useState(1);
    const [previousSorting, setPreviousSorting] = useState({ value: 'installs', label: 'Most Popular' });
    ...
    const isPathInsideDirectory = (childPath, parentPath) => {
        ...
        const relative = window.electron.pathRelative(normalizedParent, normalizedChild);
        return relative === '' || (!relative.startsWith('..') && !window.electron.pathIsAbsolute(relative));
    };
    ...
    useEffect(() => {
        ...
        const fetchSettings = async () => {
            try {
                const response = await axios.get(`${WEB_URL}/wp-json/wp/v2/site-settings`);
    ...
    useEffect(() => {
        const initializeGameDirectory = async () => {
            ...
            try {
                ...
                const sanitizedGamePath = window.electron.pathNormalize(window.electron.pathResolve(gamePath));

                // Remove the executable from the path if present
                const sanitizedPath = sanitizedGamePath.replace(/\\[^\\]+\.exe(\.exe)?$/i, "");
    ...
    useEffect(() => {
        const fetchAllAddons = async () => {
            try {
                ...
                do {
                    const { data: batchAddons, totalPages: fetchedTotalPages } = await fetchAddons(`${currentExpansion}`, currentPage, '', [], pageSize);
    ...
    const fetchAddonsData = useCallback(async (page = 1, search = '', categories = [], orderby = selectedSorting.value) => {
        try {
            setLoading(true);
            const { data, totalPages } = await fetchAddons(`${currentExpansion}`, page, search, categories, pageSize, orderby, 'desc');
    ...
    const fetchAuthorInfo = async (authorId) => {
        try {
            const { data } = await axios.get(`${WEB_URL}/wp-json/wp/v2/public-user-info/${authorId}`);
    ...
    useEffect(() => {
        if (activeTab === 'browseAddons' && searchQuery === '') {
            fetchAddonsData(currentPage, '', selectedCategories.map(option => option.value));
        } else if (activeTab === 'myAddons') {
            if (gameDir) {
                checkInstalledAddons(gameDir);
            }
        }
    }, [activeTab, currentPage, selectedCategories, fetchAddonsData, searchQuery]);

    // Load the addon categories
    useEffect(() => {
        const fetchCategories = async () => {
            try {
                const response = await axios.get(`${WEB_URL}/wp-json/wp/v2/${currentExpansion}-addon-categories`);
    ...
    const renderAddonAuthor = (addon) => {
        ...
        const avatarUrl = isAuthorCreator && authorData
    ...
    const handleSortingChange = async (selectedOption) => {
        ...
        await fetchAddonsData(1, searchQuery, selectedCategories.map(option => option.value), selectedOption.value);
    };
    ...
    const handleSearchChange = (event) => {
        ...
        if (value === '') {
            ...
            fetchAddonsData(previousPage, '', selectedCategories.map(option => option.value), previousSorting.value);
        } else {
    ...
    const handleViewAddon = async () => {
        if (contextMenu.addon) {
            ...
            try {
                const response = await axios.get(`${WEB_URL}/wp-json/wp/v2/${currentExpansion}-addons/${contextMenu.addon.id}`);
    ...
    const normalizeTitle = (title) => {
        ...
        let normalizedTitle = decodedTitle
            ...
            .replace(/classic|retail|tbc|shadowlands|2024|2023|[-\s]*ace\d*[-\s]*/gi, '') // Remove common suffixes
    ...
    const handleInstallAddon = async (addon, event, isReinstall = false) => {
        ...
        if (!gameDir) {
            showToastMessage("Your game directory is not configured for this expansion.", 'danger');
            return;
        }

        const addonUrl = addon.custom_fields.file;
        if (!addonUrl) {
            showToastMessage("The source URL for this addon couldn't be found.", 'danger');
            return;
        }

        const absoluteGameDir = window.electron.pathResolve(gameDir);
        const installPath = window.electron.pathJoin(absoluteGameDir, 'Interface', 'AddOns');
        const addonTitle = normalizeTitle(addon.custom_fields.title_toc || addon.title);
        ...
        try {
            ...
            if (isVariation) {
                ...
                const mainAddonResponse = await axios.get(`${WEB_URL}/wp-json/wp/v2/${currentExpansion}-addons/${isVariation}`);
                ...
                relatedAddons = parseSerializedPHPArray(addon.custom_fields.has_variations);
            }
            ...
            await Promise.all(addonsToDelete.map(async (relatedAddonId) => {
                const installedAddon = Object.values(installedAddons).find(installed => installed.id === parseInt(relatedAddonId, 10));

                if (installedAddon) {
                    const foldersToDelete = installedAddon.custom_fields.folder_list.map(([folderName]) => folderName);
                    ...
                    await Promise.all(foldersToDelete.map(async (folder) => {
                        ...
                        if (!isPathInsideDirectory(folderPath, installPath)) {
                            console.error('Attempted path traversal attack detected:', folderPath);
                        ...
                        for (let attempt = 0; attempt < 2; attempt++) {
                            ...
                            if (!(await window.electron.fileExists(folderPath))) {
                                ...
                                console.warn(`Attempt ${attempt + 1} to delete folder ${folderPath} failed. Retrying...`);
                        ...
                        if (!deleteSuccess) {
                            showToastMessage(`Failed to delete folder: ${folderPath}`, 'danger');
                        }
                    }));
                }
            }));
            ...
            const zipFilePath = await window.electron.saveZipFile(response.data, fileName);
            await window.electron.extractZip(zipFilePath, installPath);

            // Step 4: Write the .warperia file with correct information
            const mainFolder = addon.custom_fields.folder_list.find(([_, isMain]) => isMain === "1");
            if (!mainFolder) {
            ...
            if (await window.electron.fileExists(tocFilePath)) {
                const versionFromToc = await window.electron.readVersionFromToc(tocFilePath) || "1.0.0";
                ...
                const warperiaContent = `ID: ${addon.id}\nFolders: ${addon.custom_fields.folder_list.map(([folderName]) => folderName).join(",")}`;
                ...
                if (writtenContent !== warperiaContent) {
                    console.error("Mismatch in written .warperia file! Expected:", warperiaContent, "but got:", writtenContent);
                } else {
                }
                ...
                showToastMessage(`"${addon.title}" installed successfully with version ${backendVersion}.`, 'success');
            } else {
                console.warn(`No .toc file found in main folder "${mainFolderName}", defaulting to version 1.0.0.`);
            }
    ...
    const checkInstalledAddons = async (gamePath) => {
        try {
            ...
            await Promise.all(addonFolders.map(async (folder) => {
                const folderPath = window.electron.normalizePath(`${gamePath}\\Interface\\AddOns\\${folder}`);
                ...
                const mainFolders = matchingAddons.map(addon =>
                    addon.custom_fields.folder_list.find(([_, isMain]) => isMain === "1")?.[0]
                );
                ...
                if (mainFolders.includes(folder)) {
                    ...
                    if (await window.electron.fileExists(warperiaFile)) {
                        ...
                        if (installedAddonId) {
                            const matchedAddon = fetchedAddons.find(addon => addon.id === parseInt(installedAddonId, 10));

                            if (matchedAddon) {
                                // Check for missing folders
                                const allAddonFolders = matchedAddon.custom_fields.folder_list.map(([relatedFolder]) => relatedFolder);
                                const missingFolders = allAddonFolders.filter(folderName => !addonFolders.includes(folderName));
                    ...
                    const hasVariations = matchingAddons.some(addon => Array.isArray(addon.custom_fields?.has_variations) && addon.custom_fields.has_variations.length > 0);
                    const isVariation = matchingAddons.some(addon => addon.custom_fields?.variation && addon.custom_fields.variation !== '0' && addon.custom_fields.variation !== '');
    ...
    const handleSelectAddon = async (selectedAddon) => {
        ...
        try {
            ...
            const mainFolder = selectedAddon.custom_fields.folder_list.find(([_, isMain]) => isMain === "1")[0];
            const warperiaFilePath = `${installPath}\\${mainFolder}\\${mainFolder}.warperia`;

            await window.electron.writeFile(
                warperiaFilePath,
                `ID: ${selectedAddon.id}\nFolders: ${selectedAddon.custom_fields.folder_list.map(([folderName]) => folderName).join(",")}`
            );
            ...
            showToastMessage(`"${decodeHtmlEntities(selectedAddon.title)}" installed successfully.`, 'success');
        } catch (error) {
            console.error("Error installing addon after selection:", error);
            showToastMessage(`Failed to install "${decodeHtmlEntities(selectedAddon.title)}".`, 'danger');
        } finally {
            setTimeout(() => {
                setDownloading(false);
                setProgress(0);
    ...
    const handleDeleteAddon = async () => {
        if (contextMenu.addon) {
            try {
                ...
                const normalizedTitle = normalizeTitle(contextMenu.addon.custom_fields.title_toc || contextMenu.addon.title || '');
                ...
                if (foldersToDelete.length > 0) {
                    ...
                    showToastMessage(`"${decodeHtmlEntities(contextMenu.addon.title)}" and related folders deleted successfully.`, 'success');
                    await checkInstalledAddons(gameDir);
                    await updateAddonUninstallStats(contextMenu.addon.id, contextMenu.addon.post_type);
                } else {
                    showToastMessage(`Failed to find folders for "${decodeHtmlEntities(contextMenu.addon.title)}".`, 'danger');
                }
            } catch (error) {
                console.error("Error deleting addon:", error);
                showToastMessage(`Failed to delete "${decodeHtmlEntities(contextMenu.addon.title)}".`, 'danger');
            }
        }
    };
    ...
    const openSwitchVariationModal = async (addon) => {
        try {
            ...
            if (addon.custom_fields?.variation && addon.custom_fields.variation !== '0' && addon.custom_fields.variation !== '') {
                // Fetch the main addon if this is a variation
                const response = await axios.get(`${WEB_URL}/wp-json/wp/v2/${currentExpansion}-addons/${addon.custom_fields.variation}`);
            ...
            for (const variationId of variationIds) {
                const response = await axios.get(`${WEB_URL}/wp-json/wp/v2/${currentExpansion}-addons/${variationId}`);
    ...
    const handleSwitchVariation = async (newVariation) => {
        ...
        try {
            ...
            const installedFolders = currentAddon.custom_fields.folder_list.map(([folderName]) => folderName);
            for (const folder of installedFolders) {
                const folderPath = window.electron.pathJoin(gameDir, 'Interface', 'AddOns', folder);
                await window.electron.deleteFolder(folderPath);
            }
            ...
            // Update UI
            showToastMessage(`Switched to variation: ${newVariation.title}`, 'success');
    ...
    const handleImportAddons = async (addonIds) => {
        try {
            for (const id of addonIds) {
                const addon = await axios.get(`${WEB_URL}/wp-json/wp/v2/${currentExpansion}-addons/${id}`);
                await handleInstallAddon(addon.data, new Event('click'));
            }
            showToastMessage("Addons imported successfully!", "success");
        } catch (error) {
            showToastMessage("Failed to import addons. Invalid export code.", "danger");
        }
    };
    ...
    const handleUpdateAddon = async (addon) => {
        try {
            ...
            const mainFolder = addon.custom_fields.folder_list.find(([_, isMain]) => isMain === "1")[0];
            ...
            const currentVersion = await window.electron.readVersionFromToc(tocFilePath);
            ...
            if (currentVersion !== backendVersion) {
                ...
                showToastMessage(`The addon is already up-to-date: v${currentVersion}`, 'info');
            }
    ...
    const handleRightClick = (event, addon) => {
        ...
        const normalizedTitle = normalizeTitle(addon?.custom_fields.title_toc || addon.title || '');

        // Check if the addon is installed by looking at installedAddons
        const isInstalled = Object.values(installedAddons).some(installedAddon => {
            const normalizedInstalledTitle = normalizeTitle(installedAddon?.title || '');
            const normalizedInstalledTocTitle = normalizeTitle(installedAddon?.custom_fields?.title_toc || '');

            return normalizedInstalledTitle === normalizedTitle || normalizedInstalledTocTitle === normalizedTitle;
        });

        // Extract addon image and name
        const addonImage = addon.featured_image ? addon.featured_image : 'public/default-image.jpg';
    ...
    const renderAddonsList = (addonsList) => {
        const combinedAddonsList =
            activeTab === "myAddons" ? { ...installedAddons, ...corruptedAddons } : addonsList;

        const uniqueAddons = Object.values(combinedAddonsList).reduce((acc, addon) => {
        ...
        return (
            <div className={`row ${activeTab === "myAddons" ? "g-4" : "g-3 row-cols-md-2 row-cols-xl-2"}`}>
                    ...
                    if (activeTab === "myAddons") {
                        return (
                            <InstalledAddonCard
                                ...
                                openSwitchVariationModal={openSwitchVariationModal}
    ...
    return (
        <div>
            <div className="container position-relative" ref={containerRef}>
                {settings && settings.enable_ads_banner_top === 1 && settings.app_ads_image_banner_top && (
                            ...
                            <div className="w-100 d-flex flex-wrap align-items-center gap-2">
                                <Tippy content="Update all addons" placement="auto">
                                    <span>
                                        <button
                                            ...
                                            disabled={Object.values(installedAddons).filter(addon => addon.localVersion !== addon.custom_fields.version).length === 0 || downloading}
                                        >
                                            {downloading ? (
                                                <>
                                ...
                                <div className="ms-auto d-flex gap-2">
                                    <div className="dropdown">
                                        ...
                                        <ul className="dropdown-menu dropdown-custom-dark" aria-labelledby="exportImportDropdown">
                                            <li>
                                                <button className="dropdown-item py-2" onClick={openExportModal}>Export Addons</button>
                                            </li>
                                            <li>
                                                <button className="dropdown-item py-2" onClick={openImportModal}>Import Addons</button>
                                            </li>
                                        </ul>
                                    </div>
                        ...
                        {initialLoading || scanningAddons ? (
                            ...
                            <>
                                {Object.keys(installedAddons).length === 0 ? (
                                    ...
                                    <>
                                        {renderAddonsList(Object.values(installedAddons)
                                            .map(installedAddon => {
                ...
                {settings && settings.enable_ads_banner_bottom === 1 && settings.app_ads_image_banner_bottom && (
                    <section className="container py-3 text-center">
                        <img
            ...
            <ContextMenu
                ...
                onReinstall={contextMenu.addon && contextMenu.isInstalled ? handleReinstallAddon : null}
                onDelete={contextMenu.addon && contextMenu.isInstalled ? handleDeleteAddon : null}
            ...
            <ToastNotifications toasts={toasts} onRemoveToast={(toastId) => setToasts(prevToasts => prevToasts.filter(t => t.id !== toastId))} />
